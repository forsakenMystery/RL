...
class Q_Learning:
    ...
    def Q_Learning(self, episodes=600, maximum_movements=200, random=0.98, discount=0.999, gamma=0.99):
        show = -1
        lol = []
        epsilon = 1
        lamda = 1
        ...
        ans = []
        flag = False
        Q = np.multiply(np.ones((self.width, self.height, self.game.ACTION_NUMBERS)), self.q)
...
def main(going):
    ...
    gold = (0, 0)
    # print("Asdoasdsjksn")
    QL = Q_Learning(width, height, obstacles, me, vampires, gold, arrows)
    # print(QL.game)
    _, start, started = QL.Q_Learning(episodes=50000)
    episode = QL.test_Q_Learning()
    print(episode)
    # width = 5
    print(episode)
    ...
    XXX = 'QLearning_output_no wampus_no obstacles_end in gold_50000_7,7_' + str(going) + '.txt'
    MyFile = open(XXX, 'w')
